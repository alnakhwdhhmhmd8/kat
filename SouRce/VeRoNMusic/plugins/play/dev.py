...
async def dev(client: Client, message: Message):
     ...
     try:
      await client.send_message(username, f"**<b>[سـورس جـاك | 𝙎𝙊𝙐𝙍𝘾𝙀 𝐉𝐀𝐊. #]</b>\nحبي في حد بينادي عليك\n{chat_title}\n╰❖ ᚐᴄʜᴀᴛ ɪᴅ : {message.chat.id}**",
      reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(f"{title}", url=f"{link}")]]))
     except:
        pass
     await message.reply_photo(
     photo=photo,
     caption=f"**<b>[سـورس جـاك | 𝙎𝙊𝙐𝙍𝘾𝙀 𝐉𝐀𝐊. #]</b>\n\n╭❖ᴅᴇᴠᚐɴᴀᴍᴇ: {name}\n│᚜❖ᴅᴇᴠᚐᴜꜱᴇʀ : @{username}\n╰❖ᚐʙɪᴏ{bio}**",
...
@app.on_message(filters.command(["جـاك"], ""), group=84444677721136)
async def yas(client, message):
    usr = await client.get_chat("wwvvwl")
    name = usr.first_name
    photo = await app.download_media(usr.photo.big_file_id)
    await message.reply_photo(photo,       caption=f"**<b>[سـورس جـاك | 𝙎𝙊𝙐𝙍𝘾𝙀 𝐉𝐀𝐊. #]</b>\n\n╭❖ᚐɴᴀᴍᴇᚐ: {name}\n│᚜❖ᚐᴜꜱᴇʀ: @{usr.username}\n│᚜❖ᚐɪᴅᚐ: `{usr.id}`\n╰❖ᚐʙɪᴏᚐ: {usr.bio}**", 
